...
from MLogit import logit
from artifactClassifier import artifactsCalibration


class p300_train(object):
    def __init__(self, fn, channels, to_freq, idx=1, csp_time=[0, 0.3], a_time=0.5):
        ...
        signal = self.data.prep_signal(to_freq, channels)
        artifacts_data = np.zeros([len(channels), self.data.sampling_frequency*a_time, len(self.tags)])
    ...
    def wyr(self, tags=None, time=[0.1, 0.5], its=10, xc_time=0.1):
        ...
        while go:
            ...
            for j, i in enumerate(new_tags):
                ...
                #b,a = ss.butter(2, 7*2.0/self.fs, btype = 'low')
    ...
    def analyze(self, signal, index, tr=0.05):
        ...
        for i in xrange(N):
            ...
            if prob >= tr:
                ...
                #print 'Liczba w buforze', i + 1, index
                return index
        if index == 1:
            print i + 1
        return -1
