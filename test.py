...
for n in xrange(1, sr+1):
    z1, z2, mu, sigma = dat.get_n_mean(n, nt, [0, 0.5], 0.05)
    #plt.hist(z2, normed=False, label='Non Targets')
    #plt.hist(z1, normed=False, alpha=.5, label='Targets')
    #plt.xlabel('Max-corr')
    #plt.ylabel('P')
    #plt.legend()
    #plt.show()
    ...
    no_sigma[n-1]=sigma
print '#######ANALIZA###########'
analyze = p300.p300analysis(trgs, no_trgs, mean, no_mu, no_sigma, left, right)
s = sp('tomek2_p300.obci')
...
for p, idx in tags:
    ...
    ix = analyze.analyze(signal, idx, tr=.98)
    if ix == 1 and idx == 1:
        tp += 1
    if ix == -1 and idx != 1:
        tn += 1
...
#dat.show_mean_CSP([0.1, 0.5])
#dat.show_mean([0.1, 0.5], 'Cz')